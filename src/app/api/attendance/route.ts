...
export async function GET(req: Request) {
  try {
    ...
    const attendanceRecords = rows.map(row => ({
        ...
        status: row.status,
        checkIn: row.check_in_time ? new Date(row.check_in_time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) : null,
        checkOut: row.check_out_time ? new Date(row.check_out_time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) : null,
    }));
